...
## ข้อเดียวที่ playbook ไม่ทำให้ — kernel ต้องมาถูกสายตั้งแต่ template

playbook **ตรวจ** เป็น task แรก ๆ **ก่อนแตะอะไรบนเครื่อง** และหยุดถ้าผิดสาย
แต่ **ไม่สลับ kernel ให้** โดยเจตนา

> **versionlock ไม่ใช่ขั้นทำมืออีกแล้ว** (12 ก.ย. 2026) — เคยเป็นด่านที่สองที่หยุด
> playbook แล้วให้คนไป `dnf versionlock add` เอง ด้วยเหตุผล "ควรอยู่ใน template"
> ซึ่งเป็นความชอบ ไม่ใช่ความเสี่ยง · มัน idempotent และถอดได้ทุกเมื่อ ต่างจากการสลับ
> kernel ที่พลาดแล้วเครื่องบูตไม่ขึ้น · ตอนนี้ playbook ลง plugin + ล็อกให้เอง
> ถ้าล็อกอยู่แล้วก็ข้าม

> **ไม่ต้อง ssh ไปดูเองก่อน** — เคยมี loop `uname -r` + `audit-node.sh` ตรงนี้ ตัดออกแล้ว
> (12 ก.ย. 2026) เพราะมันซ้ำกับ gate ในตัว playbook เป๊ะ ๆ · gate อยู่ก่อน task แรก
> ที่แก้เครื่อง (`ตั้ง hostname`) จึงไม่มีทางที่เครื่องจะถูกแก้ครึ่งเดียวแล้วค่อยรู้ว่า
> ไม่พร้อม · **รัน playbook ไปเลย** ถ้าขาดข้อไหนมันหยุดที่ task นั้นพร้อมบอกว่าต้องทำอะไร
>
> loop ทำมือมีค่าตอน playbook ยังพิสูจน์ตัวเองไม่ได้ (ขั้น 1-5 ข้างล่าง) พอผ่านแล้ว
> มันกลายเป็นพิธีกรรมที่ทำให้คนถามว่า "แล้วผมจะใช้ Ansible ทำไม"
...
🔴 **เลือกเส้นทางตามสถานะตั้งต้นก่อน — สองแบบนี้คนละลำดับกัน**

| สถานะตั้งต้น | ใช้เส้นทาง |
|---|---|
| มีเครื่องที่**ทำมือตามบท 01 เสร็จแล้ว** อย่างน้อยหนึ่งเครื่อง | ขั้น 1-5 ข้างล่าง |
| **เครื่องเปล่าทั้งหมด** — ลง OS ใหม่ / cluster ใหม่ | [เส้นทางเครื่องเปล่า](#เส้นทางเครื่องเปล่า--ยังไม่มีเครื่องไหนทำมือ) |

---

### เส้นทางเครื่องเปล่า — ยังไม่มีเครื่องไหนทำมือ

ขั้น 1-5 ตั้งอยู่บนสมมติฐานว่ามีเครื่องที่ทำมือไว้เทียบ **ถ้าไม่มี สมมติฐานนั้นหายไปทั้งชุด**
· `--check` ใส่เครื่องเปล่าจะล้มเองที่ task แรกที่ต่อเนื่องกัน (ดู
[ตารางในหัวข้อแก้ปัญหา](#--check-ล้มบนเครื่องเปล่า--dest--must-be-an-existing-dir))
**อย่าเริ่มด้วย `--check`**

ขอแค่ kernel มาถูกสาย (`6.12` + `uek`) จาก template — ที่เหลือ playbook ทำหมด:

```bash
# 1 · master01 เครื่องเดียวก่อน — รันจริงเลย ไม่ต้อง --check
ansible-playbook prepare-os.yml --limit k8s-master01
```

```bash
# 2 · ผ่านแล้ว --check ซ้ำเครื่องเดิม — ตอนนี้มันมีความหมายแล้ว
#     เครื่องอยู่ในสถานะปลายทาง จึงเป็นการทดสอบว่า playbook idempotent จริง
ansible-playbook prepare-os.yml --check --diff --limit k8s-master01 --skip-tags reboot
```

**ต้องได้ `changed=0`** — ถ้ายังมี `changed` แปลว่ามี task ที่รายงานผิดหรือแก้ซ้ำทุกรอบ
...
> **ไม่ใส่ `--skip-tags reboot` ตอนรันจริง** — playbook reboot ทีละเครื่อง (`serial: 1`)
> แล้ว assert ซ้ำหลังบูตให้เอง · ที่ใส่ `--skip-tags reboot` มีแค่ตอน `--check`

---

### ขั้น 1 · ทำมือหนึ่งเครื่องก่อน ✅

> ขั้น 1-5 คือ**เส้นทางที่มีเครื่องทำมือไว้เทียบ** — ถ้าเครื่องเปล่าทั้งหมด
> ให้ใช้ [เส้นทางเครื่องเปล่า](#เส้นทางเครื่องเปล่า--ยังไม่มีเครื่องไหนทำมือ) ข้างบนแทน
...
ไม่คุม  : หางเลข 105/203/206        → errata ของ Oracle ไม่กระทบ Cilium
```

`KERNEL_UEK` ใน `versions.env` จึงเป็นแค่ `6.12` — playbook เทียบ `startswith("6.12.")`
ทั้งก่อนและหลัง reboot · เคยเก็บเลขเต็มแล้วเจอว่า 6 เครื่องได้ errata คนละตัวตามวันที่
`dnf update` ทำให้ตรงกับใครก็ไม่ครบ (11 ก.ย. 2026) — เลิกคุมหางเลขที่นี่

**"6 เครื่องต้องเลขเดียวกัน" ยังเป็นเป้าหมายอยู่** แต่เป็นหน้าที่ของ alert
`NodeKernelVersionMismatch` (บท 09) ไม่ใช่ของ playbook · สถานะและแผนอยู่ใน
[CHECKLIST หมวด C2](../docs/CHECKLIST.md)
