...
**ควรเห็น:** `6.12.0-204.92.4.3.1.el9uek.x86_64` — **ต้องตรงกับ `$KERNEL_UEK` เป๊ะ**

> ถ้าเครื่องนี้ได้เลขอื่น แปลว่า ISO/template ที่ใช้ให้ kernel คนละ patch level
> **หยุดแล้วตัดสินใจก่อน** ว่าจะยึดเลขไหนทั้งโปรเจกต์ แล้วแก้ที่ `versions.env` ที่เดียว
> อย่าปล่อยให้แต่ละเครื่องเป็นคนละเลข
