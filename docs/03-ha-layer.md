...
> **ลำดับ: ข้อ 0-3 ทำทีละเครื่องให้จบ — master01 → master02 → master03 · ข้อ 4-5 รอครบ 3 เครื่องก่อน**
> **เวลาที่ใช้:** ~20 นาที + ทดสอบ failover ~15 นาที
> **⚠️ บทนี้ต้องผ่าน failover test ก่อนไปบทที่ 04 เด็ดขาด**

> ทำไมทีละเครื่อง: ข้อ 3 มี config คนละไฟล์ต่อเครื่อง (`priority` 110/100/90) — เปิด 3 terminal
> ทำข้อละทีพร้อมกันคือวิธีที่วางไฟล์ผิดเครื่องบ่อยที่สุด · ทำไม master01 ก่อน: มันมี priority
> สูงสุด VIP จะขึ้นที่มันตรง ๆ ไม่แวะไปอยู่เครื่องอื่นชั่วคราวให้งง (keepalived ตั้ง preempt
> ไว้ ลำดับจึงไม่ทำให้ผลสุดท้ายต่าง แต่ทำให้ระหว่างทางอ่านง่ายกว่า)
...
**ต้องมีก่อน:** `/root/k8s/` บนเครื่องนี้ (copy มาตั้งแต่[บท 00](00-overview.md)) · ทำทุกครั้งที่เปิด shell ใหม่
...
**ต้องมีก่อน:** [บท 01](01-prepare-os.md) จบ (repo ของ OS เปิดแล้ว) · ไม่พึ่งข้อ 0
...
**ต้องมีก่อน:** ข้อ 1 (มี `/etc/haproxy/` แล้ว) · ไฟล์เดียวกันทั้ง 3 เครื่อง
...
**ต้องมีก่อน:** ข้อ 0 (ตัวแปร `$VIP`) · ข้อ 1 · **ข้อ 2 บนเครื่องเดียวกันต้องเสร็จและ HAProxy
รันอยู่** — health check ของ keepalived ([`check_apiserver.sh`](../config/keepalived/check_apiserver.sh)
ด่าน 1) ตรวจว่า HAProxy ฟัง `:8443` ถ้ายังไม่รัน keepalived จะลด priority ตัวเองทันทีที่ขึ้น
...
**⏸ ต้องมีก่อน: ข้อ 3 จบครบทั้ง 3 เครื่อง** — ข้อนี้ตรวจสองอย่างพร้อมกัน (VIP อยู่ที่ master01
*และ* ไม่อยู่ที่ 02/03) ถ้าเครื่องไหนยังไม่ขึ้น ผลที่เห็นจะไม่ใช่ผลจริง
...
**ต้องมีก่อน:** ข้อ 4 ผ่าน (VIP อยู่ที่ master01 ครบ 3 เครื่องยืนยันแล้ว) · 5.1-5.3 ทำได้เลย
· **5.4 ต้องมี cluster จาก[บท 04](04-create-cluster.md) ก่อน** — กลับมาทำหลังบท 04
...
**ทำที่:** ssh เข้า 👑 master01 **สองหน้าต่าง** (มันมีทั้ง `kubectl` จากบท 04 ข้อ 3 และถือ VIP อยู่)
· บล็อกที่มี `ssh root@...` รันจาก**เครื่องคุณ** (WSL ที่แลก key ครบ 6 เครื่อง)
· **ต้องมีก่อน:** บท 04 จบ — `kubectl get nodes` บน master01 เห็นครบ 6 เครื่อง

5.1-5.3 พิสูจน์ได้แค่ว่า **keepalived ย้าย IP เป็น** — ข้อนี้พิสูจน์ว่า **ย้ายแล้ว `kubectl` ยังใช้ต่อได้**
ซึ่งเป็นสิ่งที่ต้องการจริง (ด่าน 3 ของ [`check_apiserver.sh`](../config/keepalived/check_apiserver.sh)
ยิง `/healthz` ผ่าน VIP — ก่อนมี cluster ด่านนี้ไม่เคยถูกรันเลย)

**1 · จากเครื่องคุณ — HAProxy ทั้ง 3 เครื่องต้องเห็น backend ครบ** ไม่งั้น VIP อาจย้ายไปเครื่องที่ส่งต่อไม่ได้:
...
**ควรเห็น:** `L7OK` ครบ 9 ช่อง (3 เครื่อง × 3 backend) · มี `L6RSP` ที่ไหนแก้ก่อน ([บท 13 ข้อ 1.3b](13-troubleshooting.md))

**2 · หน้าต่างที่ 2 บน master01 — เปิดค้างไว้ วน `kubectl` ผ่าน VIP ทุกวินาที:**
...
**ควรเห็น:** `ok` ทุกบรรทัด · ปล่อยไว้แล้วไปหน้าต่างที่ 1

**3 · หน้าต่างที่ 1 บน master01 — ดึงปลั๊ก keepalived:**

```bash
systemctl stop keepalived
```

**4 · ดูหน้าต่างที่ 2 — เกณฑ์ผ่านที่แท้จริง:** ขาดได้ไม่เกิน **2-3 บรรทัด** แล้วกลับมา `ok` เอง
โดยไม่แตะ kubeconfig ไม่รีสตาร์ตอะไร · **นับบรรทัดที่ขาดแล้วจด** — นั่นคือ downtime ของ control plane
ตอน failover ที่ต้องตอบทีม เดาไม่ได้ ต้องวัด

**5 · หน้าต่างที่ 1 — คืนค่า แล้วจากเครื่องคุณยืนยันว่า VIP กลับ master01:**
...
**ควรเห็น:** `1` ที่ master01 เครื่องเดียว · อีกสองเครื่อง `0` · ปิดหน้าต่างที่ 2 ได้ (Ctrl-C)
