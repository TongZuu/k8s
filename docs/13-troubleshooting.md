...
เกิดเฉพาะเครื่องที่ย้าย partition มาเป็น `/var/lib/etcd` (บท 01 ข้อ 5 — เป็นทางเลือก)
เครื่องที่ไม่ได้ย้ายจะไม่มีโฟลเดอร์นี้เลยตอน init/join ซึ่ง kubeadm สร้างให้เองและไม่มีทางเจอ error นี้ ·
ext4 แถม `lost+found` มาให้ทุก partition ใหม่ และถ้าเครื่องนั้นเคยมีคนใช้ `/home`
